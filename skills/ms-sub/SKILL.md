...
- 订阅前同时检查 `rssId` 和 `archived`：
    - `rssId > 0` 视为已经订阅
    - `archived = true` 视为已经入库
    - 只有 `rssId = 0` 或缺失，且 `archived != true` 时，才继续新增订阅
...
- 输出至少包含：
    ...
    - 类型
    - 季数
    - 状态，显示中文描述（100: 订阅就绪, 200: 订阅运行中, 300: 订阅已完成）
    - TMDB
...
- 订阅前不要只看 `rssId`，还要一起考虑 `archived` 是否已入库。
- 调用 `subscribe add` 时，优先使用 TMDB 结果里的规范 `title` 和 `year`。
